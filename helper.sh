...
PROJECT_NAME="testDMS"
STACK_NAME="testDMS"
TEMPLATE_NAME="root.yaml"
...
upload()
{
    ...
    aws s3 cp ./ s3://${TEMPLATE_BUCKET}/testDMS --recursive  --exclude "*.sh" --exclude "*.json" --include "*.yaml" --exclude "root.yaml" --profile ${AWS_PROFILE}
...
if [ $1 == "test" ]; then 
    ...
    aws rds describe-db-engine-versions --query DBEngineVersions[?Engine==$engine].EngineVersion  --profile hardis
    exit $?
fi
